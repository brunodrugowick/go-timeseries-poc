...
// server is a simple server structure to control startup and shutdown of our HTTP server(s)
type server struct {
	server http.Server
}

// Run simply runs the server and do a proper shutdown in case of an interrupt signal
func (s *server) Run() {
	ctx := context.Background()

	go func() {
		if err := s.server.ListenAndServe(); err != http.ErrServerClosed {
			log.Fatalf("Failed starting server with %v\n", err)
		}
	}()
	...
	<-stop
	log.Printf("[%s] I'm going down...", s.server.Addr)
...
// Build returns a server with the current config from defaultServerBuilder
func (b *defaultServerBuilder) Build() server {
...
func defaultHandler(writer http.ResponseWriter, request *http.Request) {
	t, _ := template.New("index").Parse("<h1>Golang server is running...</h1>\n\n<p>This is the default home page if you don't configure your server.")
	data := struct {
		Path string
	}{
